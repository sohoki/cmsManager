package aten.com.backoffice.sym.log.service;
...
public class SysLogAspect    {
	...
	@Around("execution(public * egovframework.let..impl.*Impl.update*(..)) "
							+ " || execution(* aten.com.backoffice..impl.*Impl.update*(..))   "
							+ " &&  !@target(aten.com.backoffice.sym.log.annotation.NoLogging) "
				            + " &&  !@annotation(aten.com.backoffice.sym.log.annotation.NoLogging) )" )	
	 ...
	 * @throws Exception
	 */
	@Around("execution(public * egovframework.let..impl.*Impl.delete*(..)) "
					+ " || execution(* aten.com.backoffice..impl.*Impl.delete*(..))   "
					+ " &&  !@target(aten.com.backoffice.sym.log.annotation.NoLogging) "
		            + " &&  !@annotation(aten.com.backoffice.sym.log.annotation.NoLogging) )" )
	public Object logDelete(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			LOGGER.debug("sqlid:" + sqlid.toString());
	    	...
	    	sysLog.setErrorCode("200");
			sysLog.setSrvcNm(joinPoint.getTarget().getClass().getName());
			sysLog.setMethodNm(joinPoint.getSignature().getName());
			sysLog.setProcessSeCode("D");
			sysLog.setProcessTime( Long.toString(stopWatch.getTotalTimeMillis()));
			sysLog.setRqesterId(uniqId);
			sysLog.setRqesterIp(ip);
			sysLogService.logInsertSysLog(sysLog);
	...
	@AfterThrowing(pointcut = "execution( public * egovframework.let..impl.*Impl.update*(..))  "
						     + " or execution(* aten.com.backoffice..impl.*Impl.update*(..)) " 
						     + " and  !@target(aten.com.backoffice.sym.log.annotation.NoLogging) "
						     + " and  !@annotation(aten.com.backoffice.sym.log.annotation.NoLogging) )", throwing = "error")
